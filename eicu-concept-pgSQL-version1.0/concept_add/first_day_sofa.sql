DROP TABLE IF EXISTS eicu_derived.first_day_sofa CASCADE;
CREATE TABLE eicu_derived.first_day_sofa AS
-- day 1 labs
with la as
(
  ...
  from eicu_derived.pivoted_lab p
...
(
  ...
  from eicu_derived.pivoted_med p
  where chartoffset <= 1440 AND chartoffset >= 0
  group by p.patientunitstayid
)
-- day 1 for infusions
...
(
  ...
  , max(vasopressin) as vasopressin
  , max(milrinone) as milrinone
  from eicu_derived.pivoted_infusion p
  where chartoffset <= 1440 AND chartoffset >= 0
  group by p.patientunitstayid
)
...
(
  ...
  from eicu_derived.pivoted_vital p
	where (chartoffset <= 1440 AND chartoffset >= 0) AND
...
(
  ...
  left join eicu.apacheapsvar aav
    on pt.patientunitstayid = aav.patientunitstayid
  left join eicu.apachepredvar apv
